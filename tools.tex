...
\begin{itemize}
	...
	\item Icomoon \cite{es4} - This is an application with the purpose of generating an icon font from Scalable Vector Graphics (SVG) files that you upload to it. You can also resize, adjust positions, and set default pixel sizes for the icons. The application turns all the icons into crisp-looking and easy scalable icons. It also automatically generates the HTML and CSS code which you can use to integrate the code into your own application. In this project, Icomoon was used to make the category icons that show the various categories on each story
...
\subsection{Back-end}
\label{subsec:back_end_tools}
\begin{itemize}
	\item PHP \cite{HM8} - PHP is a server-side scripting language produced by The PHP Group that is especially suited for web development. It is a general-purpose scripting language often used to provide dynamic content from a web server to a client.
	\item Java \cite{HM9} - Java is a very popular general-purpose object oriented programming language developed by Sun Microsystems and later Oracle Corporation.
...
\begin{itemize}
	...
	\item \LaTeX\ \cite{as10} - \LaTeX\ is a document preparation system. Used to produce the final version of the report. Chosen because it provides control over sectioning, cross-referencing, tables and figures. The code for the report was hosted by GitHub to ease cooperation.
	\item TeXstudio \cite{as11} - TeXstudio is an integrated writing environment used to create \LaTeX\ documents. This tool was chosen because it has a user interface which simplifies the work.
\end{itemize}
