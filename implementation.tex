...
Project progression is derived from the activity plan ref{app:activityplan}, meeting minutes \todo{ref{app:group-meeting}} and Trello archive \todo{Blir dette referet til? + legg inn ref her. Står det noe om hvor lange sprintene er tidliger? når de slutter of started klokkeslett}  and will only briefly summarize what was done at each sprint and only serves as a "quick tour" through the development process  
...
\begin{description}
	...
	We dealt with the feedback from the test conducted during the passover \todo{Blir dette riktig?}. Some of the problems where platform specific needed some focus to hammer out. Did some more polish on the  views and cleared out some front-end bugs. on back-end work continued on filtering both collaborative and content-based which it would for the rest of the sprints. 
