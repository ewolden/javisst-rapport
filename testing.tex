...
The unit tests and integration test were required to be executed and approved before the system test could be executed. To execute, the tester needed the final product in the way it was intended to the end users. Therefore the application needed to be deployed on a mobile device. \newline
...
Some of the test cases have a dependability of other tests. If an issue is detected in one test case, it might cause issues in its dependent test cases. \textbf{Table \ref{Tab:systemtest1}} and \textbf{Table \ref{Tab:systemtest2}} are presenting two of the test cases that were used. The whole test case document are in Appendix C in tables C1.-C.9. 
...
\begin{table}[H]
	...
	\begin{table}[H]
		...
		\begin{tabular}{ | l | l  |}
			...
			\paragraph{Summary}
			-hvordan ble det egentlig utført
